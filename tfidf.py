...
for f in all_files:
    ...
    doc_words    = open(f,errors='ignore').read().lower()
    #print 'words:\n', doc_words
...
for f in all_files:
    #fPath=f.split('/')
    #/home/saurabhska/cs590/rework/Analysis-of-Research-Trends/tf-idf/NounFiles/NounFiles/Nouns2014-2015/nounnewAbstract65657.txt
    #fPath[-2]+='_tfidf'
    #f='/'.join(fPath)
    #newPath=fPath
    #del fPath[-1]
    ...
    #fParts=f.split('.')
    if len(fParts) > 1:
      writer = open(f.split('.')[0] + '_tfidf' +'.'+ f.split('.')[1], 'w')
    else:
    ...
    for (tfidf, term) in result[:top_k]:
        if (display_mode == 'both' and tfidf >0):
            writer.write(term + '\t')
        elif (tfidf > 0):
            writer.write(term + '\t')
